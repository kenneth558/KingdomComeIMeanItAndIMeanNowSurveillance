...
#         sleep 40
         ...
         echo "   Southwest corner camera:" > "$mailingdirname1/subject.txt"
         ...
         echo "   Front parking lot camera:" > "$mailingdirname2/subject.txt"
         ...
         echo "   East driveway camera:" > "$mailingdirname3/subject.txt"
         ...
         echo "   South driveway camera:" > "$mailingdirname4/subject.txt"
         ...
         echo "   Front northeast corner camera:" > "$mailingdirname5/subject.txt"
...
while [[ "$(ps x|grep -v grep|grep 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.12:9554/Streaming/channels/101.*-c copy')." != "." ]]; do
    kill $(ps x|grep -v grep|grep -m1 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.12:9554/Streaming/channels/101.*-c copy'|awk '{printf $1}') 2>/dev/null
    sleep 1
done
while [[ "$(ps x|grep -v grep|grep 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.13:554/Streaming/channels/101.*-c copy')." != "." ]]; do
    kill $(ps x|grep -v grep|grep -m1 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.13:554/Streaming/channels/101.*-c copy'|awk '{printf $1}') 2>/dev/null
    sleep 1
done
while [[ "$(ps x|grep -v grep|grep 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.14:7554/Streaming/channels/101.*-c copy')." != "." ]]; do
    kill $(ps x|grep -v grep|grep -m1 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.14:7554/Streaming/channels/101.*-c copy'|awk '{printf $1}') 2>/dev/null
    sleep 1
done
while [[ "$(ps x|grep -v grep|grep 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.15:7554/Streaming/channels/101.*-c copy')." != "." ]]; do
    kill $(ps x|grep -v grep|grep -m1 'ffmpeg -rtsp_transport udp -i rtsp://kERRY123:Kerry123@192.168.0.15:7554/Streaming/channels/101.*-c copy'|awk '{printf $1}') 2>/dev/null
    sleep 1
done
...
/var/www/camera_streams/camera_east_driveway/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null &
sleep 1
nohup ffmpeg -rtsp_transport udp -i "rtsp://kERRY123:Kerry123@192.168.0.12:9554/Streaming/channels/101" -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 \
-c:v libx264 -f segment -segment_time 360 -segment_format_options movflags=+faststart -reset_timestamps 1 -avoid_negative_ts 1 -c copy -flags +global_header \
/var/www/camera_streams/camera_front_parking/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null &
sleep 1
nohup ffmpeg -rtsp_transport udp -i "rtsp://kERRY123:Kerry123@192.168.0.13:554/Streaming/channels/101" -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 \
-c:v libx264 -f segment -segment_time 360 -segment_format_options movflags=+faststart -reset_timestamps 1 -avoid_negative_ts 1 -c copy -flags +global_header \
/var/www/camera_streams/camera_south_driveway/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null &
sleep 1
nohup ffmpeg -rtsp_transport udp -i "rtsp://kERRY123:Kerry123@192.168.0.14:7554/Streaming/channels/101" -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 \
-c:v libx264 -f segment -segment_time 360 -segment_format_options movflags=+faststart -reset_timestamps 1 -avoid_negative_ts 1 -c copy -flags +global_header \
/var/www/camera_streams/camera_sw_corner/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null &
sleep 1
nohup ffmpeg -rtsp_transport udp -i "rtsp://kERRY123:Kerry123@192.168.0.15:7554/Streaming/channels/101" -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 \
-c:v libx264 -f segment -segment_time 360 -segment_format_options movflags=+faststart -reset_timestamps 1 -avoid_negative_ts 1 -c copy -flags +global_header \
/var/www/camera_streams/camera_ne_corner/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null &

nohup inotifywait -m /var/www/camera_streams/ -r -e create 2>/dev/null|while read pathandcamera action file;do if [[ -f "$pathandcamera""$file" ]] && [ "${file%-*}" != "$(date +%Y%m%d)" ] && [ "${file##*.}" == "mp4" ];then currentprocessid=$(ps x|grep -v grep|grep -m1 "$pathandcamera"|awk '{printf $1}');${SHELL} -c "nice -11 $(ps aux|grep -v grep|grep -m1 "$pathandcamera"|awk '{out=$11;lastfield=NF;for (i=12;i<lastfield;i++){out=out" "$i}{printf out" "}}';echo "$pathandcamera$(date +%Y%m%d-)%d.mp4 2>/dev/null &")";kill $currentprocessid;touch /tmp/new_day_no_backup_yet;elif [[ $(($((360 * $(date +%H))) + $((60 * $(date +%M))) + $(date +%S))) -gt 1000 ]] && [[ -f /tmp/new_day_no_backup_yet ]];then rm /tmp/new_day_no_backup_yet;/home/homeowner/backupndaysago 3;fi 1>/dev/null 2>/dev/null & done 1>/dev/null 2>/dev/null &

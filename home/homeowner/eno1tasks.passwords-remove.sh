...
#if [[ $(ps auxf|grep -v grep|grep -c "inotifywait -mr /etc/emails_awaiting/ -e create") == 0 ]];then nohup inotifywait -mr /etc/emails_awaiting/ -e create 2>/dev/null|while read path action file; do 
#   if [[ "$file" == "recipients.txt" ]];then 
#      until cat "$path"content|sudo -u "$(cat "$path"subject.txt|awk '{printf $1}')" mail -s "$(cat "$path"subject.txt)" "$(cat "$path""$file")" 2>/dev/null; do sleep 189;done && 
#        [[ -f "$path"attachmentsforremainingemails.txt ]] && 
#        while read -r jpgnametoattach;do sudo -u "$(cat "$path"subject.txt|awk '{printf $1}')" /home/homeowner/smtp-cli --server=smtp.gmail.com:587 -4 --user=<insert your email user name here> --pass=<insert your password here> $(cat "$path""picturerecipients.txt"|awk -F, '{for (x=1;x<=NF;x++){printf "--to="$x" "}}') --from="$(cat "$path"subject.txt|awk '{printf $1}')"@gwaamc.com --subject="$(cat "$path"subject.txt)" --attach="$jpgnametoattach" 2>/dev/null;done < "$path"namesofjpgstoattach.txt;rm -r "$path" 2>/dev/null;fi;done & fi
...
#     while read -r jpgnametoattach;do sudo -u "$(cat "$path"subject.txt|awk '{printf $1}')" /home/homeowner/smtp-cli --server=smtp.gmail.com:587 -4 --user=<insert your email uuser name here> --pass=<insert your password here> $(cat "$path""picturerecipients.txt"|awk -F, '{for (x=1;x<=NF;x++){printf "--to="$x" "}}') --from="$(cat "$path"subject.txt|awk '{printf $1}')"@gwaamc.com --subject="$(cat "$path"subject.txt)" --attach="$jpgnametoattach" 2>/dev/null;done < "$path"namesofjpgstoattach.txt;rm -r "$path" 2>/dev/null;fi;done &
...
#just_recover:
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:6080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:6080,fork,reuseaddr TCP:192.168.0.11:6080 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:6554,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:6554,fork,reuseaddr TCP:192.168.0.11:6554 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:9080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:9080,fork,reuseaddr TCP:192.168.0.12:9081 2>/dev/null & fi
...
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:10080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:10080,fork,reuseaddr TCP:192.168.0.13:7080 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:10554,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:10554,fork,reuseaddr TCP:192.168.0.13:554 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:14080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:14080,fork,reuseaddr TCP:192.168.0.14:7080 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:14554,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:14554,fork,reuseaddr TCP:192.168.0.14:7554 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:15080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:15080,fork,reuseaddr TCP:192.168.0.15:7080 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:15554,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:15554,fork,reuseaddr TCP:192.168.0.15:7554 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:16080,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:16080,fork,reuseaddr TCP:192.168.0.16:6080 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:16554,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:16554,fork,reuseaddr TCP:192.168.0.16:6554 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:21100,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:21100,fork,reuseaddr TCP:192.168.0.100:80 2>/dev/null & fi
#if [[  $(ps auxf|grep -v grep|grep -c 'socat -T300 TCP4-LISTEN:21253,') == 0 ]];then nohup socat -T300 TCP4-LISTEN:21253,fork,reuseaddr TCP:192.168.0.253:80 2>/dev/null & fi
...
#sleep 1
#if [[  $(ps auxf|grep -v grep|grep -c ffmpeg.*\.16\:) == 0 ]];then bash -c 'nohup ffmpeg -nostdin -stimeout 10000000 -rtsp_transport udp -i "rtsp://192.168.0.16:7554/Streaming/channels/101" -reconnect 1 -reconnect_at_eof 1 -reconnect_streamed 1 -c:v libx264 -f segment -segment_time 180 -segment_format_options movflags=+faststart -reset_timestamps 1 -avoid_negative_ts 1 -c copy -flags +global_header /var/www/camera_streams/camera_spare0/$(date +%Y%m%d-%H%M%S)%d.mp4 > /dev/null 2>/dev/null & ';fi
#sleep 1
#if [[  $(ps auxf|grep -v grep|grep -c ffmpeg.*\.100\:) == 0 ]];then nohup bash -c 'while :;do sudo date +%Y_%b_%d_%H:%M:%S.%N|while read stamp;do ffmpeg -i http://192.168.0.100:80/stream  -movflags=+faststart frag_keyframe -min_frag_duration 12000000 -c copy /camera_snapshots/camera_esp32100/100_"$stamp"%d.mp4;done;if [[ $(( $(ls /camera_snapshots/camera_esp32100|wc -l) )) -ge 1000 ]];then rm /camera_snapshots/camera_esp32100/"$(ls -t /camera_snapshots/camera_esp32100|tail -n1)";fi;done' 1>/dev/null 2>/dev/null & fi

#if [[  $(ps auxf|grep -v grep|grep -c ffmpeg.*\.100\:) == 0 ]];then nohup bash -c 'while :;do sudo date +%Y_%b_%d_%H:%M:%S.%N|while read stamp;do ffmpeg -i http://192.168.0.100:80/stream -movflags frag_keyframe -min_frag_duration 10000000 -y -c copy /camera_snapshots/camera_esp32100/100_"$stamp".mp4;done;done' 1>/dev/null 2>/dev/null & fi
...
#/home/homeowner/startdaemons.sh

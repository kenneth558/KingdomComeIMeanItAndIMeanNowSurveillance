...
#        while read -r jpgnametoattach;do sudo -u "$(cat "$path"subject.txt|awk '{printf $1}')" /home/homeowner/smtp-cli --server=smtp.gmail.com:587 -4 --user=gwaamcfirewall --pass=post-ISIS4024311ken $(cat "$path""picturerecipients.txt"|awk -F, '{for (x=1;x<=NF;x++){printf "--to="$x" "}}') --from="$(cat "$path"subject.txt|awk '{printf $1}')"@gwaamc.com --subject="$(cat "$path"subject.txt)" --attach="$jpgnametoattach" 2>/dev/null;done < "$path"namesofjpgstoattach.txt;rm -r "$path" 2>/dev/null;fi;done & fi
